...
- Images for score items (in priority order):
...
Usage (local test):
  python wisco_core_teams_rss_MEDIA4_JPG.py --out wisconsin-sports.xml --days 6 --max-items 240 --live-final-only --prefer-event-photos --scrape-og --fallback-base-url "https://<user>.github.io/wisconsin-sports-feed"
...
def pick_event_image_from_json(event: dict) -> str | None:
...
def event_to_item(event: dict, league_label: str, live_final_only: bool, logo_urls: dict,
                  prefer_event_photos: bool = False, scrape_og: bool = False) -> dict | None:
    comp = (event.get("competitions") or [{}])[0]
    competitors = comp.get("competitors") or []
    ...
    if not image and logo_urls:
        for key, url in logo_urls.items():
            if key.lower() in (home_name + " " + away_name).lower():
                image = url
                break
...
def build_rss(items: list, title="Wisconsin Sports (Core Teams)", link="https://www.espn.com/",
              description="Packers, Bucks, Brewers scores + UW Badgers scores & news.",
    ...
    for it in items:
        ...
        # CDATA-safe description: neutralize any embedded ']]>'
...
def main():
    ...
    ap.add_argument("--fallback-base-url", default="", help="Base URL where /img logos live (e.g., https://<user>.github.io/wisconsin-sports-feed)")
