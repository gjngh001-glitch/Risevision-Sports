...
#!/usr/bin/env python3
# Wisconsin Sports Aggregator RSS - Packers, Bucks, Brewers, Badgers (MEDIA5 ASCII)
# ------------------------------------------------------------------------------
...
# - Image priority for score items:
#     1) ESPN JSON team logos/photos
#     2) Optional: scrape og:image from event link
#     3) JPG fallback logos (packers.jpg, bucks.jpg, brewers.jpg, badgers.jpg)
# - Emits both <enclosure> and <media:content>; stable GUIDs to avoid image bleed.
# - CDATA-safe description (sanitizes any embedded ']]>' in news HTML).
#
# Usage (local test):
#   python wisco_core_teams_rss_MEDIA5_ASCII.py --out wisconsin-sports.xml --days 6 --max-items 240 --live-final-only --prefer-event-photos --scrape-og --fallback-base-url "https://<user>.github.io/risevision-sports"
...
TEAM_MATCHERS = [
    ...
    "Wisconsin Badgers", "Wisconsin", "UW", "UW-Madison",
]
...
IMG_TAG_RE = re.compile(r'<img[^>]+src=["\']([^"\'\s>]+)["\']', re.IGNORECASE)
...
def fetch_og_image(url: str, timeout: int = 8) -> str:
    try:
        ...
        return None
    m = re.search(r'<meta[^>]+property=["\']og:image["\'][^>]+content=["\']([^"\'\s>]+)["\']', html, re.IGNORECASE)
...
def pick_event_image_from_json(event: dict) -> str:
...
def event_to_item(event: dict, league_label: str, live_final_only: bool, logo_urls: dict,
                  prefer_event_photos: bool = False, scrape_og: bool = False) -> dict:
    comp = (event.get("competitions") or [{}])[0]
    competitors = comp.get("competitors") or []
    ...
    image = None
    if prefer_event_photos:
        image = pick_event_image_from_json(event)
    if not image and scrape_og:
        image = fetch_og_image(link)
    if not image and logo_urls:
        for key, url in logo_urls.items():
            names_joined = (home_name + " " + away_name).lower()
            if key.lower() in names_joined:
                image = url
                break
    ...
    return {
        ...
        "guid": stable_guid(link, pubdate or "", title),
        "kind": "score",
        "image": image,
    }
...
def parse_team_rss(feed_title: str, url: str, max_items: int = 25):
    ...
    for item in root.findall(".//item"):
        ...
        items.append({
            ...
            "guid": stable_guid(link or title, pub, feed_title or ""),
            "kind": "news",
            "image": image,
        })
...
def build_rss(items: list, title="Wisconsin Sports (Core Teams)", link="https://www.espn.com/",
              description="Packers, Bucks, Brewers scores + UW Badgers scores and news.",
              media_namespace=True) -> str:
    now = format_datetime(dt.datetime.now(dt.timezone.utc))
    ns_attr = ' xmlns:media="http://search.yahoo.com/mrss/"' if media_namespace else ""
    buf = []
    buf.append('<?xml version="1.0" encoding="UTF-8"?>')
    buf.append(f'<rss version="2.0"{ns_attr}>')
    buf.append("<channel>")
    ...
    for it in items:
        ...
        if it.get("image"):
            img = escape(it["image"])
            buf.append(f"<enclosure url=\"{img}\" type=\"image/jpeg\" />")
            buf.append(f"<media:content url=\"{img}\" medium=\"image\" />")
        safe_desc = (it.get('description') or "").replace(']]>', ']]]]><![CDATA[>')
        buf.append(f"<description><![CDATA[{safe_desc}]]></description>")
        buf.append("</item>")
    buf.append("</channel>")
    buf.append("</rss>")
    return "\n".join(buf)
...
def main():
    ...
    ap.add_argument("--max-items", type=int, default=240, help="Max items in the final RSS feed")
    ...
    ap.add_argument("--fallback-base-url", default="", help="Base URL where /img logos live (e.g., https://<user>.github.io/risevision-sports)")
    ap.add_argument("--prefer-event-photos", action="store_true", help="Use ESPN team/event images for score items")
    ap.add_argument("--scrape-og", action="store_true", help="Fetch og:image from event page for score items")
    args = ap.parse_args()
        ...
        for d in date_range(today, args.days):
            ...
            for event in data.get("events", []):
                item = event_to_item(
                    event,
                    label,
                    live_final_only=args.live_final_only,
                    logo_urls=logo_urls,
                    prefer_event_photos=args.prefer_event_photos,
                    scrape_og=args.scrape_og,
                )
                if item:
                    items.append(item)
